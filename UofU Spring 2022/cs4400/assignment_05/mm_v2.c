...
static void *set_allocated(void *b, size_t size)
{
  ...
  {
    // sets the current block's alloc bit
  ...
  else
  {
    ...
    PUT(b, PACK(GET_SIZE(HDRP(b)), set_alloc_bit(b)));

    // set next block pointer's previous alloc bit     
    PUT(NEXT_BLKP(b), PACK(GET_SIZE(HDRP(NEXT_BLKP(b))), set_prev_alloc_bit(NEXT_BLKP(b))));
  }
