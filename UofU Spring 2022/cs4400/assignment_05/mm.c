...
void *mm_malloc(size_t size)
{
  ...
  while (1)
  {
    ...
    if (GET_SIZE(HDRP(freeList_iter)) >= newsize)
    {
      ...
      /* void *copyfreeHead = freeHead; */
      /* while (((freeList *)copyfreeHead) != NULL) */
      /* { */
      /* 	printf("current free block: %p  | cfb next: %p | cfb prev: %p\n", copyfreeHead, ((freeList *)copyfreeHead) -> next, ((freeList *)copyfreeHead) -> prev); */
      /* 	copyfreeHead = ((freeList *)copyfreeHead) -> next; */
      /* } */
...
void mm_free(void *ptr)
{
  ...
  PUT(HDRP(ptr), PACK(GET_SIZE(HDRP(ptr)), clear_alloc_bit(HDRP(ptr))));
  add_to_free_list(ptr);

  // coalesce the recently unalloc'd block with surrounding unalloc'd blocks if available
...
static void *set_allocated(void *b, size_t size)
{
  ...
  if (new_BlockSize >= (8 * SMALLEST_BLOCKSIZE))
  {
...
static void* coalesce(void *bp)
{
  ...
  int prevPro = 0;
  int nextTerm = 0;
  //Checks to see if the previous 8 bytes is NOT the prolog
  if (IS_PROLOG(HDRP(bp) - sizeof(PADDING)) != 4)
  {
    //checks to see if the previous block is unalloc
  ...
  else if (IS_PROLOG(HDRP(bp) - sizeof(PADDING)) == 4)
  {
    prevPro = 1;
  }
  
  //Checks to see if next 8 bytes is NOT the term block
  if (GET_SIZE(FTRP(bp) + sizeof(term)) != 0)
  {
    //checks to see if the next block is unalloc
  ...
  else if (GET_SIZE(FTRP(bp) + sizeof(term)) == 0)
  {
    nextTerm = 1;
  }

  // bp is the only block in the page, so no need to coalesce
  if (prevPro && nextTerm)
  {
    return bp;
  }
...
static void remove_from_free_list (void * ptr)
{
  ...
  printf("before removing %p from the list\n", ptr);
  while (((freeList *)copyfreeHead) != NULL)
  {
    printf("current free block: %p  | cfb next: %p | cfb prev: %p\n", copyfreeHead, ((freeList *)copyfreeHead) -> next, ((freeList *)copyfreeHead) -> prev);
    copyfreeHead = ((freeList *)copyfreeHead) -> next;
  ...
  {
    ((freeList *)prevBlock) -> next = ((freeList *)ptr) -> next;
    ((freeList *)nextBlock) -> prev = ((freeList *)ptr) -> prev;
  }
  ...
  ((freeList *)ptr) -> next = NULL;
  ((freeList *)ptr) -> prev = NULL;

  
  printf("after removing %p from the list\n", ptr);
  copyfreeHead = freeHead;
  while (((freeList *)copyfreeHead) != NULL)
  {
    printf("current free block: %p  | cfb next: %p | cfb prev: %p\n", copyfreeHead, ((freeList *)copyfreeHead) -> next, ((freeList *)copyfreeHead) -> prev);
...
void checkHeap(){
  ...
  void *currPage = first_ever_page;
  void *block_iter = first_ever_page + sizeof(pageList) + sizeof(PADDING) + sizeof(block_header);
  int startOfNewpage = 1;


  printf("Current state of the page list\n");
  void *currentpg = first_ever_page;
  while (currentpg != NULL)
  {
    printf("current page: %p | NP: %p| PP: %p\n\n", currentpg, ((pageList*)currentpg) -> next, ((pageList *)currentpg) -> prev);
    currentpg = ((pageList *)currentpg) -> next;
  }
  ...
  while (currPage != NULL)
  {
    ...
    if (startOfNewpage)
    {
      ...
      printf("| pageList | PADDING |");
      startOfNewpage = 0;
    }
    
    
    printf("| H: S:%d A:%d PA:%d |", GET_SIZE(HDRP(block_iter)), GET_ALLOC(HDRP(block_iter)), GET_PREV_ALLOC(HDRP(block_iter)));
    printf("| pay |");
    printf("| F: S:%d |", GET_SIZE(FTRP(block_iter)));

    if (GET_SIZE((FTRP(block_iter) + sizeof(size_t))) == 0)
    {
      printf("| term |\n\n");
      currPage = ((pageList *)currPage) -> next;
      
      if (currPage != NULL)
      {
	block_iter = currPage + sizeof(pageList) + sizeof(PADDING) + sizeof(block_header);
	startOfNewpage = 1;
	continue;
      }
      else
