...
class EntityA:
    ...
    def output(self, message):
        ...
        if (self.currSeq == 0):
            ...
            self.currSeq = 0
        # Add the recently added packet to the queue
    ...
    def input(self, packet):
        ...
        #checks that the acknum is 0 (aka an ACK) and the packet's seq num matches with the packet sent at the top of the queue
        if (len(self.pktQ) != 0) and (localCS == packet.checksum) and (packet.seqnum == self.pktQ[0].seqnum):
            ...
            #send next packet on top of the queue if not empty
            if (len(self.pktQ) != 0):
                start_timer(self, self.timer)
                to_layer3(self, self.pktQ[0])
                self.state = 1
    ...
    def timer_interrupt(self):
        ...
        # resent the last packet (aka the first packet in the queue)
...
class EntityB:
    ...
    def input(self, packet):
        ...
        if (localchksm == packet.checksum):
            # Checks Sequence numbers, if not matching send last ACK
            if (self.seqNum == packet.seqnum):
                
                retPkt = Pkt(self.seqNum, 0, (0 + self.seqNum), packet.payload)
                ...
                retPkt = Pkt(prevSeq, 0, prevSeq, packet.payload)

        # if corrupt at all send NAK, if mismatched seq nums, send previous ACK
        else:
