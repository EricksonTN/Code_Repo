...
class EntityA:
    ...
    def __init__(self, seqnum_limit):
        ...
        # Sequence Num Limit
        self.seqLim = seqnum_limit

        # Queue for packets ready to be sent
        self.pktQ = []

        # Index of the next packet to be sent in the pktQ (nextseqnum equivalent in the diagram)
        self.nextPkt = 0
        ...
        # Window Size
        self.winSZ = int(seqnum_limit / 2)

        # Window Base
        self.winBS = 0

        #Packets in Flight data structure
        self.PIF =[]
    ...
    def output(self, message):
        ...
        # Formulate the pkt with the current Sequence number, the current acknum, the checksum, and the message itself
        pkt = Pkt(self.currSeq, 0, checksumCalc(self.currSeq, 0, message), message.data)

        # Add the recently created packet to the queue
        self.pktQ.append(pkt)

        # start the timer
        if (self.winBS == self.nextPkt):
            start_timer(self, self.timer)

        # Send pkts up to the window size.
        self.sendMoreIP()

        # inc currSeq
        if (self.currSeq == self.seqLim - 1):
            self.currSeq = 0
        else:
            self.currSeq += 1
    ...
    def input(self, packet):
        # Local CheckSum
        localCS = packet.seqnum + packet.acknum
        
        #Checks for corruption
        if localCS == packet.checksum:
            
            # If the received packet's seq num exists in the packets in flight array, do work.
            if packet.seqnum in self.PIF:

                # Inc window base
                self.winBS += self.PIF.index(packet.seqnum) + 1

                pktIndex = self.PIF.index(packet.seqnum)

                # Handle Cumulative acks, remove pkts in PIF prior to the received pkt's seq num
                for j in range(pktIndex + 1):
                    self.PIF.pop(0)
    ...
    def timer_interrupt(self):
        ...
        for i in range(self.winSZ):
            resendIndex = self.winBS + i
            if resendIndex == len(self.pktQ):
                break
            resendPkt = self.pktQ[resendIndex]

            
            #if not (resendPkt.seqnum in self.PIF):
            self.PIF.append(resendPkt.seqnum)

            to_layer3(self, resendPkt)

    
    # Helper Function, sends packets up to the window size or till the last element in the queue.
    def sendMoreIP(self):

        for x in range(self.winSZ):
            pktInd = self.winBS + x
            if pktInd < len(self.pktQ):

                if not (self.pktQ[pktInd].seqnum in self.PIF):
                    
                    to_layer3(self, self.pktQ[pktInd])
                    self.PIF.append(self.pktQ[pktInd].seqnum)
...
class EntityB:
    ...
    def input(self, packet):
        ...
        # Local Checksum
        localchksm = checksumCalc(packet.seqnum, packet.acknum, Msg(packet.payload))
        retPkt = None
        # Check the checksum
        
        # != means corruption, == means no corruption
        if (localchksm == packet.checksum) and (self.expSeq == packet.seqnum):
            
            retPkt = Pkt(self.expSeq, 0, (0 + self.expSeq), packet.payload)
            
            #Set the last in order pkt rcv'd
            self.seqRcv = retPkt

            # inc seqNum
            if (self.expSeq == self.seqLim - 1):
                self.expSeq = 0
            else:
                self.expSeq += 1
            ...
            # Set retPkt to the last in-order packet received.
            retPkt = self.seqRcv

        to_layer3(self, retPkt)
    ...
    def timer_interrupt(self):
        ...
        pass
# Checksum calculator method, will be dissolved and functionality duplicated in Entity A and B
def checksumCalc (seq, ack, message):
    chksm = int.from_bytes(message.data, 'big')
    chksm += seq + ack
