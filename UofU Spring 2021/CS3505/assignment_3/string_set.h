...
namespace cs3505
{
  class string_set
    ...
    private:
      ...
      /* Cleans up the string_set and deallocates all the nodes.     
       */
      void clean ();

      // Traverses the list to find the position prior to insertion
      std::vector<node *> listSearch (const std::string & data) const;

      // Copies the set over to this
      void copySet (const string_set &  set);
