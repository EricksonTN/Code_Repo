...
namespace cs3505
{
  ...
  class node 
  {
    ...
    // Students must decide what functions and variables are needed here.
      ...
      int node_width;           // Integer value of the max width of the node
      std::string data;         // String value that contains the data of the node
      std::vector<node *> next; // Next list that contains pointers

      
      node (const std::string & data, int max_width);
      ~node();
  };
}
