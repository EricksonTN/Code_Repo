...
/* 
 * 'combined_setting' objects represent a setting that
 * is both stored remotely and observed, such as a
 * graphical setting in a game.  
 *
 * As an example:
 *
 * A graphical setting may be changed locally, or 
 * updated from a remote server, so this class inherits 
 * from server_setting.  Also, any changes to a graphical 
 * setting might need to be immediately drawn to the screen,
 * so this class inherits from notify so that the objects
 * responsible for drawing the screen can be notified if
 * the setting changes.
 */
class combined_setting : virtual public notify, virtual public server_setting
{
public:
  combined_setting (string v);
  virtual ~combined_setting ();

  virtual void change (string v);
};

/* Constructor:  combined_setting::combined_setting
 *
 * Simply constructs the superclass portions of
 * this object.
 */
combined_setting::combined_setting (string v)
  : server_setting(v), notify(v), setting(v)
{
  cout << "  ==> combined_setting::combined_setting" << endl;
  cout << "  <-- combined_setting::combined_setting" << endl;
}

/* Destructor:  combined_setting::~combined_setting ()
 *
 ...
 */
combined_setting::~combined_setting ()
{
  cout << "  ==> combined_setting::~combined_setting" << endl;
  cout << "  <-- combined_setting::~combined_setting" << endl;
}
 ...
 */
void combined_setting::change(string v)
{
  cout << "  ==> combined_setting::change" << endl;
...
int main ()
{
  ...
  cout << "Creating a combined_setting object:" << endl;
  cout << "-----------------------------------" << endl;

  combined_setting * c_setting = new combined_setting("low_resolution");
  ...
  vector<combined_setting*> vector_c;
  vector<server_setting*>   vector_r;
  vector<notify*>           vector_n;
  vector<setting*>          vector_s;

  vector_c.push_back(c_setting);
  vector_r.push_back(c_setting);
  vector_n.push_back(c_setting);
  vector_s.push_back(c_setting);
  ...
  // First, use a combined_setting pointer.

  cout << "Setting the user setting to \"foggy\" (through a combined_setting *):" << endl;
  cout << "---------------------------------------------------------------------" << endl;

  combined_setting *t = vector_c[0];
  t->change("foggy");
  ...
  cout << endl;
  cout << "combined setting is now " << result << "." << endl; 
  cout << endl;
