...
class CurveDrawer {
	constructor()
	{
		...
		this.attribBuf = gl.createBuffer();
		gl.bindBuffer(gl.ARRAY_BUFFER, this.attribBuf);
...
var curvesVS = `
	...
	void main()
	{
		...
		vec2 curve = (pow(1.0 - t, 3.0) * p0) + (3.0 * pow(1.0 - t, 2.0) * t * p1) + (3.0 * (1.0 - t) * (pow(t, 2.0)) * p2) + (pow(t, 3.0) * p3);
		gl_Position = mvp * vec4(curve,0,1);
	}
`;
